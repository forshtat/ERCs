...
This document describes the validation rules that participants of Account Abstraction protocols must follow for user transactions represented as `UserOperation` structs, alongside the rationale for each rule. Block builders and standalone bundlers enforce these rules off-chain.

This document is able to define the rules applicable for both [ERC-4337](./eip-4337) UserOperations and [EIP-8141](./eip-8141.md) Frame Transactions despite their differences by first establishing a shared vocabulary and terminology for Account Abstraction. 
...
This document uses the term "UserOperation" for a transaction created by a smart contract account, following [ERC-4337](./eip-4337) terminology. [EIP-8141](./eip-8141.md)'s Frame Transactions introduce a native transaction type, but face the identical DoS-safety problem. Most rules in this document apply to both models. Where a rule's mechanism is specific to one Account Abstraction model, it is marked accordingly.
...
| `SAME_SENDER_MEMPOOL_COUNT`          | 4                           | ERC-4337 only. Maximum number of UserOperations allowed in the mempool from a single sender. |
...
| `MAX_VERIFY_STATE_GAS`               | 500000                      | EIP-8141 only. Maximum state gas ([EIP-8037](./eip-8037.md)) budgeted across a frame transaction's Validation Phase. ERC-4337's legacy single-dimensional gas model has no equivalent to meter. |
...
### WIP WIP TBD Terminology Mapping

This document's rules are written in terms of abstract entities and mechanisms — `account`, `factory`, `paymaster`etc. — that originate
from [ERC-4337](./eip-4337.md)'s `UserOperation` model. This section defines how each term resolves for an [EIP-8141](./eip-8141.md) Frame Transaction.

| Term (as used throughout this document) | ERC-4337 `UserOperation` meaning                                                                                                                                                        | EIP-8141 frame transaction meaning                                                                                                                                                                                                                                                                                                                                                                                                                          |
|-----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **account** / **sender**                | The `sender` field's Smart Contract Account.                                                                                                                                            | `tx.sender` — specifically, the `resolved_target` of the `VERIFY`-mode frame that successfully calls `APPROVE(APPROVE_EXECUTION)` or `APPROVE(APPROVE_EXECUTION_AND_PAYMENT)` (EIP-8141 §APPROVE Instruction requires `resolved_target == tx.sender` for either).                                                                                                                                                                                           |
| **paymaster**                           | The `paymaster` field's contract, if present.                                                                                                                                           | The `resolved_target` of the `VERIFY`-mode frame that successfully calls `APPROVE(APPROVE_PAYMENT)` or `APPROVE(APPROVE_EXECUTION_AND_PAYMENT)` — i.e. the `pay` or `self_verify` mode subclass (EIP-8141 §Mode Subclassifications). When a single `self_verify` frame handles both scopes, **account and paymaster coincide**, exactly matching the ERC-4337 case of an empty `paymaster` field ("sender pays for gas by itself").                         |
| **factory**                             | The `factory`/`factoryData` fields' contract, if present.                                                                                                                               | The `resolved_target` of the first `deploy`-mode frame (`DEFAULT` mode, first frame in the validation prefix) — see EIP-8141 §Mode Subclassifications and §Public Mempool-recognized Validation Prefixes.                                                                                                                                                                                                                                                   |
| **aggregator**                          | An authorizer contract enabling shared validation across multiple `UserOperation`s (ERC-4337 §Definitions).                                                                             | **Not defined.** A frame transaction carries exactly one `sender` field (EIP-8141 §Payload Encoding); there is no substrate for cross-sender signature aggregation. Every rule below referencing an aggregator is marked *(ERC-4337 UserOperation only)*.                                                                                                                                                                                                   |
| **EntryPoint** (as a contract)          | A singleton, externally deployed contract exposing `handleOps`, a deposit ledger (`balanceOf`/`depositTo`/`withdrawTo`), and a stake ledger (`addStake`/`unlockStake`/`withdrawStake`). | **Not defined as a contract.** `ENTRY_POINT` (`address(0xaa)`, EIP-8141 §Constants) is a protocol constant, observed only as a frame's `caller`, returned by `ORIGIN` in `DEFAULT`/`VERIFY`-mode frames. It holds no ledger. Rules that depend on calling the `EntryPoint` contract are marked *(ERC-4337 UserOperation only)*.                                                                                                                             |
| **`UserOperation`**                     | The pseudo-transaction struct itself.                                                                                                                                                   | A frame transaction (`FRAME_TX_TYPE = 0x06`), when this document's rules are being applied to that transaction type.                                                                                                                                                                                                                                                                                                                                        |
| **`validateUserOp` call**               | A named interface method on the account.                                                                                                                                                | Not a named call — the code a `VERIFY`-mode frame executes before calling `APPROVE(APPROVE_EXECUTION[...])`.                                                                                                                                                                                                                                                                                                                                                |
| **`validatePaymasterUserOp` call**      | A named interface method on the paymaster.                                                                                                                                              | Not a named call — the code a `VERIFY`-mode frame executes before calling `APPROVE(APPROVE_PAYMENT)`.                                                                                                                                                                                                                                                                                                                                                       |
| **context** (paymaster-returned bytes)  | An ABI `bytes` value returned by `validatePaymasterUserOp`, later passed to `postOp`.                                                                                                   | **Not defined.** Data that would flow through `context` instead flows through `FRAMEDATALOAD`/`FRAMEDATACOPY` introspection of other frames' already-signed `data` fields (EIP-8141 §Introspection). Rules referencing `context` are marked *(ERC-4337 UserOperation only)*.                                                                                                                                                                                |
| **deposit**                             | A balance held by the `EntryPoint` contract, debited by `handleOps`.                                                                                                                    | The payer's own account balance, debited directly by `APPROVE` (EIP-8141 §APPROVE Instruction) — there is no separate ledger to hold a deposit in.                                                                                                                                                                                                                                                                                                          |
| **stake**                               | Tracked by the `EntryPoint` contract's `addStake`/`unlockStake`/`withdrawStake`, subject to `MIN_STAKE_VALUE`/`MIN_UNSTAKE_DELAY`.                                                      | Tracked by a separate **Staking Registry** contract (not part of the EIP-8141 protocol surface — see [Appendix A](#appendix-a-staking-registry-contract)), since `ENTRY_POINT` holds no ledger. The same `MIN_STAKE_VALUE`/`MIN_UNSTAKE_DELAY` thresholds apply.                                                                                                                                                                                            |
| **bundle** / **`handleOps` call**       | The batched transaction a bundler submits, containing multiple `UserOperation`s.                                                                                                        | **Not defined.** A frame transaction is itself the L1 transaction (EIP-8141 §Payload Encoding); there is no outer call batching several senders' operations together. Rules referencing bundle assembly are marked *(ERC-4337 UserOperation only)*; where a rule's *intent* still applies with "bundle creation" reread as "this node's own attempt to get the transaction included in a block," that reading is noted inline rather than forking the rule. |
| **first/second/third validation**       | A bundler validates a `UserOperation` once before mempool admission ("first"), again immediately before including it in a `handleOps` bundle ("second"), and validates the assembled bundle as a whole before submission ("third") — see [Running the Validation Rules in ERC-4337](#running-the-validation-rules-in-erc-4337). The second and third passes exist to protect the bundler's own signed, self-paid batch transaction from going stale between admission and submission. | **Not defined.** A frame transaction is itself the signed, self-paying L1 transaction (see "bundle" above) - there is no bundler-owned batch transaction for a second or third pass to protect, and consequently no discrete "second validation"/"third validation" checkpoint distinct from ordinary mempool admission. Rules whose mechanism depends on comparing state across two such passes are marked *(ERC-4337 UserOperation only)*. |
...
### Running the Validation Rules in ERC-4337
...
This section's title is scoped to ERC-4337 deliberately: this whole multi-pass procedure - and the "first"/"second"/"third validation" vocabulary it defines - has no EIP-8141 equivalent. See Terminology Mapping, "first/second/third validation".
...
* Opcodes that access information outside of storage and code, referred to here as the "environment", are blocked:
    * **[OP-011]** Blocked opcodes:
        * `ORIGIN` (`0x32`) (ERC-4337 UserOperation only)
        * `GASPRICE` (`0x3A`)
        * `BLOCKHASH` (`0x40`)
        * `COINBASE` (`0x41`)
        * `TIMESTAMP` (`0x42`)
        ...
        * `GASLIMIT` (`0x45`)
        * `BASEFEE` (`0x48`)
        * `BLOBHASH` (`0x49`) (ERC-4337 UserOperation only)
        * `BLOBBASEFEE` (`0x4A`)
        ...
        * `INVALID` (`0xFE`)
        * `SELFDESTRUCT` (`0xFF`)
        * `SETDELEGATE` (`0xF6`, [EIP-7819](./eip-7819.md)) *(new; except inside a Frame Transaction's `deploy` frame)*
        * `SLOTNUM` (`0x4B`, [EIP-7843](./eip-7843.md)) *(new)*
...
* Allowed access to the `EntryPoint` address *(ERC-4337 UserOperation only)*:
...
* **[COD-010]** Between the first and second validations, the `EXTCODEHASH` value of any visited address,
  ...
  *(ERC-4337 UserOperation only — see Terminology Mapping, "first/second/third validation".)*
...
* Access to associated storage of the account in an external contract that is not an entity is allowed if either:
    ...
    * **[STO-022]** There is an `initCode` and the `factory` contract is staked.
      *(For an EIP-8141 frame transaction, read `initCode` as "a `deploy` frame is present" — EIP-8141 has no `initCode` field; see Terminology Mapping, "factory.")*
...
* **[GREP-040]** If an entity fails bundle creation after passing the second validation, its `opsSeen` is set to `BAN_OPS_SEEN_PENALTY` and its `opsIncluded` to zero, causing it to become `BANNED`.
  *(For an EIP-8141 frame transaction, read "fails bundle creation" as "fails this node's own attempt to get the transaction included in a block" — see Terminology Mapping, "bundle." No rewording of the penalty mechanism itself is needed.)*
...
* **[SREP-010]** The "standard mempool" defines an entity as staked if it has at least `MIN_STAKE_VALUE` and an unstake delay of at least `MIN_UNSTAKE_DELAY`.
...
* **[EREP-015]** A `paymaster` should not have its opsSeen incremented because of a failure of the factory or account.
  *(ERC-4337 UserOperation only — see Terminology Mapping, "first/second/third validation": the correction this rule makes only has something to trigger on because a second validation pass exists to fail.)*
...
* **[EREP-016]** An `aggregator` should not have its opsSeen incremented because of a failure of a factory, an account, or a paymaster.
  *(ERC-4337 UserOperation only — doubly so: see Terminology Mapping, "aggregator" and "first/second/third validation".)*
...
* **[EREP-040]** An `aggregator` must be staked, regardless of storage usage. (ERC-4337 UserOperation only)
* **[EREP-050]** An unstaked `paymaster` may not return a `context`. (ERC-4337 UserOperation only)
* **[EREP-055]** A `context`'s size may not change between validation and bundle creation. (ERC-4337 UserOperation only)
...
* **[EREP-070]** During bundle creation, if a staked entity reduces its validation gas by more than 10%
    compared to the second validation, that entity is throttled, even if the UserOperation itself did not revert, since this might affect the gas calculation defined by [EIP-7623](./eip-7623). *(ERC-4337 UserOperation only — see Terminology Mapping, "bundle" and "first/second/third validation".)*
...
An alternate mempool is an agreed-upon rule that bundlers may opt into, in addition to the standard mempool.
...
* **[ALT-020]** The alt-mempool rules MUST be checked only when a standard rule is violated.
    * That is, if validation follows the standard rules above, it is not considered part of an alt-mempool.
*  **[ALT-021]** Such a `UserOperation`, since it violates the standard rules, is checked against all the alt-mempools, and is considered part of all those alt-mempools.
...
Alt-mempools are served by the same bundlers participating in the standard mempool, but change the rules and may introduce denial-of-service attack vectors. To prevent them from taking the standard mempool or other alt-mempools down with them, a reputation is managed for each. An alt-mempool that causes too many invalidations gets throttled. This limits the scope of the attack and lets the bundler continue doing its work for other mempools.
...
### Authorizations in ERC-4337 UserOperations 
...
### Limitations in ERC-4337 UserOperations
...
The following rules address [EIP-8141](./eip-8141.md) Frame Transaction mechanisms with no [ERC-4337](./eip-4337.md) analogue.

* **[FRA-010]** A frame transaction's validation prefix MUST match one of the recognized shapes defined in EIP-8141. A frame transaction whose validation prefix matches none of these shapes MUST be rejected. These are:
  * `[self_verify]`
  * `[deploy, self_verify]`
  * `[only_verify, pay]`
  * `[deploy, only_verify, pay]`
* **[FRA-020]** No frame in the validation prefix may carry `ATOMIC_BATCH_FLAG`. Atomic batches are permitted only among `SENDER` and `DEFAULT` frames after the validation prefix.
* **[FRA-030]** An `expiry_verify` frame may appear only as the first frame in the transaction. A node MUST drop a Frame Transaction whose `expiry_verify`
  deadline has already passed from the mempool.
* **[FRA-040]** The sum of `limits.execution` across the validation prefix, plus the intrinsic cost of validating `tx.signatures`, MUST NOT exceed `MAX_VERIFY_GAS`. The sum of `limits.state` across the validation prefix MUST NOT exceed `MAX_VERIFY_STATE_GAS`.
* **[FRA-050]** `CREATE`, `CREATE2`, and `SETDELEGATE` are permitted only inside the first `deploy` frame of the validation prefix, and only when installing code or an EIP-7702-style delegation indicator at `tx.sender`.
* **[FRA-060]** Every entry in `tx.signatures` MUST validate successfully against `compute_sig_hash(tx)`, either for its explicit 32-byte `msg` digest or an empty `msg`.
...
For EIP-8141 frame transactions this document's staking provisions require a separate contract.
 
It is deployed at address `STAKE_MANAGER_FRAMETX_ADDRESS` and implements the following Solidity interface:
...
interface IStakingRegistry {
    ...
    function getDepositInfo(address account) external view returns (uint256 stake, uint32 unstakeDelaySec, uint64 withdrawTime);
}
```
