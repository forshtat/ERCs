...
* **[EREP-070]** During bundle creation, if a staked entity reduces its validation gas by more than 10%
    compared to the second validation, that entity is throttled, even if the UserOperation itself did not revert, since this might affect the gas calculation defined by [EIP-7623](./eip-7623). *(ERC-4337 UserOperation only — see Terminology Mapping, "bundle": an EIP-8141 frame transaction has no bundle-assembly step for a staked entity to game between its second validation and inclusion, matching the reasoning already given for the third validation pass in [Running the Validation Rules](#running-the-validation-rules).)*
