...
- `data` is set to encoding of `(bytes32 challenge, uint64 revokedAt)`.
...
4. The verifier checks the response by confirming `refUID` equals `attestationUID` and `data.challenge` equals the `challenge` generated in step 1. If all checks pass it treats zero `data.revokedAt` as valid and non-zero `data.revokedAt` as revoked.
