...
A descriptor MAY cover a single function. Such a *function descriptor* is self-contained, so that a constrained verifier can check it, together with its attestation, without fetching any other file.
...
Some verifiers, such as hardware wallets, have too little memory to hold every descriptor of a large contract. Attesting a descriptor per function lets such a verifier receive, hash and check only the function a transaction uses.
...
A **function descriptor** is a descriptor that covers exactly one function of a contract, or one primary type of an [EIP-712](./eip-712.md) message, and is self-contained. It:

1. contains no `includes` key: every included file has already been resolved into it, so the distributed file is the resolved descriptor `D'` of [Descriptor Hash Computation](#descriptor-hash-computation);
2. carries its complete `context` binding as [ERC-7730](./eip-7730.md) defines it — the chain and address of every deployment, or the factory, EIP-712 deployment or domain separator it applies to;
3. contains exactly one display format entry, together with every definition, enum, constant and metadata entry that entry needs and nothing else.

This ERC assumes ERC-7730 permits a descriptor of this form.

A function descriptor's `descriptorHash` is computed exactly as above. Because it contains no `includes`, its hash covers the bytes the verifier receives. Attesters SHOULD distribute a function descriptor in its RFC 8785 canonical form, so that a memory-limited verifier can hash the received bytes directly.

A verifier that verifies a function descriptor:

- MUST reject a descriptor that contains an `includes` key or more than one display format entry;
- MUST confirm that the embedded `context` binding and the descriptor's single format match the transaction or message being displayed — the chain, the address, and the function selector or EIP-712 primary type.

The attestation signs the `descriptorHash`, which covers the embedded context and format. An attestation over a function descriptor therefore cannot be replayed for another function or another contract.

The maximum size of a function descriptor is out of scope for this ERC.
...
2. Choose a TTL and compute `expirationTime` (see [Lifecycle Guidance](#lifecycle-guidance)).
   Set `time` to the actual time of signing (see [Issue Time](#issue-time)).
...
6. Check revocation against the canonical EAS contract's state (queried directly or via trusted infrastructure that mirrors it):
   ...
   If the attestation was obtained through a registry that defines additional revocation rules, the verifier MUST apply those rules as well (see [Revocation](#revocation)).
...
EAS revocation invalidates one attestation everywhere. A registry MAY define additional, narrower revocations — for example the revocation of one function at one deployment context, which leaves the same attestation valid elsewhere. Such a revocation is defined by that registry and applies in addition to EAS revocation.

### Issue Time

An attestation's `time` field is its signed issue time. Registries MAY compare it with revocation timestamps that they record outside EAS. To make that comparison meaningful, attesters:

- MUST set `time` to the time of signing, in seconds, and MUST NOT backdate it;
- MUST give an attestation that corrects a revoked descriptor a `time` later than the recorded revocation timestamp.
...
### Why Function Descriptors

The descriptor hash is already computed over the resolved descriptor, but a verifier that receives a file with `includes` cannot check that hash without fetching the included files too. A function descriptor ships resolved, so the file the device receives is the file that was attested, and it is limited to one function so that its size does not grow with the contract. Embedding the complete context inside the signed content is what lets a device tie a small attestation to the exact contract and function it is displaying.
...
This ERC is independent of [ERC-7730](./eip-7730.md) in that attestations are a separate document type. Whole-contract descriptors and their existing attestations remain valid and are verified as before. Function descriptors assume that ERC-7730 permits a descriptor covering a single function or EIP-712 primary type.
...
| Cross-function or cross-contract replay | Attacker presents an attested function descriptor as the descriptor for a different function, or for the same function of a different contract. | The signed `descriptorHash` covers the embedded `context` and the single format. The verifier MUST match both against what is being displayed. |
| Unbounded fetching for a function descriptor | Attacker ships a "function descriptor" with `includes` so that a constrained verifier must fetch and trust further files. | Verifiers MUST reject a function descriptor that contains an `includes` key. |
